...
É possível configurar metadados a nível de repositório, que serão herdados por todas coleções no repositório. Da mesma forma, coleções configuradas como filha herdam todos metadados da coleção apontada.
  ...
  * Consulte a seção Coleções para saber mais sobre a configuração de coleções herdadas.
  * Também é possível importar e exportar metadados predefinidos (Consulte Ações em Massa)
...
* Metadados que não foram herdados do repositório (ou de coleções acima) podem ser excluídos, apresentado um ícone de lixeira à frente. (Consulte Excluir metadados.)
...
!> Atenção: Para criar novos termos na lista de seleção é necessário que o usuário tenha permissão para a edição do metadado em questão. (Consulte Criar Taxonomias para saber como permitir a adição de novos termos durante o envio de itens)
...
1. Crie uma taxonomia. (consulte Criar Taxonomias);
...
Atualmente o Tainacan tem seis tipos de metadados e filtros por padrão, mas novos conjuntos podem ser desenvolvidos de acordo com a necessidade individual de cada instalação e/ou adicionados ao core do Tainacan. (Consulte o artigo sobre o Plugin-dev Desenvolvimento do Plugin para saber mais.
...
5. Clique no tipo de metadado que deseja criar (Consulte Tipos de metadados para saber o metadado mais adequado para cada tipo de informação);
  * Cada metadado criado tem um conjunto de configurações e funções individuais para configuração:
    ...
    4. Status: determina o nível de privacidade do metadado, por exemplo metadados para informações sensíveis, como Estado de conservação ou Valor da obra.
      ...
      * Visível apenas para editores: são exibidos apenas para administradores, editores e moderadores da coleção. (Consulte Permissões de Usuários para saber mais detalhes)
  ...
  * Dependendo do tipo de metadado, também existem opções específicas a serem configuradas (Consulte Tipos de metadados para mais detalhes)
...
2. Na barra lateral esquerda clique em Tainacan e então:
  ...
  2. Edite Metadados a nível de Coleções.
    ...
    * Escolha a coleção que deseja editar os metadados (Consulte Criar coleção para saber mais);
...
3. Excluir Metadados a nível de Repositório:
  ...
  2. Clique no ícone de lixeira à direita do metadado que deseja excluir.
    * Atenção: Ao clicar na lixeira o metadado é excluído imediatamente.
    * Atenção: Esta operação não poderá ser desfeita.
    * Atenção: A exclusão de metadados a nível de repositório afetam todos dados já preenchidos e informações em itens que já foram criados em todo o repositório.
    * Atenção: Antes da exclusão do metadado, recomenda-se que este seja somente desabilitado (até que haja certeza de que a exclusão não implicará na perda de informações).
    * Atenção: Exclua o metadado somente se tiver certeza de que não haverá perdas graves de informação. (Consulte Registro de Atividades para saber mais)
4. Excluir Metadados a nível de Coleções.
  ...
  * Atenção: A exclusão destes metadados também afeta itens coleções filhas desta coleção.
  1. Escolha a coleção que deseja editar os metadados (Consulte Criar coleção para saber mais);
...
5. Na tela de edição de Metadados (a nível de repositório ou a nível de coleção), é possível visualizar os metadados já criados (ou herdados) na coluna à esquerda.
  1. Clique no ícone de lixeira à direita do metadado que deseja excluir.
...
!> Atenção: a exclusão do metadado implica também na exclusão de seus valores em todos os itens aos quais se aplica. Esta operação só pode ser desfeita por administradores. (Consulte Registro de atividades para saber mais)
