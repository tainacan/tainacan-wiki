...
* **Máquina Fotográfica** - *Tipo Relacionamento* - A máquina usada para a foto;
* **Categoria** - *Tipo Taxonomia* - A categoria da foto;

Para a Máquina Fotográfica, escolhi um *metadado tipo relacionamento* pois tenho uma coleção já configurada, com várias máquinas cadastradas. É interessante para mim que seja assim porque cada máquina possui diversas características (valores de zoom, abertura, modelo) - ou seja, seus próprios metadados - que não precisam estar registradas aqui na minha coleção de fotos, mas podem ser referenciadas.

Já para a Categoria configurei um metadado tipo taxonomia. Por quê isso? Bom, cabe pensar, o que é uma categoria? Para minha coleção, é um conjunto de termos que uso para descrever uma foto, que podem ter relações hierárquicas, como categorias mães e categorias filhas, além de surgirem novas de acordo com as novas fotos que cheguem à minha coleção. Isso cai como uma luva no que definimos como Taxonomias no Tainacan. Mas para de fato criar este metadado, antes eu preciso configurar esta Taxonomia.

## Criando a taxonomia Categorias

No menu do repositório, ao lado esquerdo, podemos acessar a lista de taxonomias e de lá criar uma nova, denominada Categoria. Após criar, posso também inserir termos, que serão as minhas categorias em si, com as quais vou rotular minhas fotos. 
...
Aqui podemos ver que o Tainacan já gerou uma miniatura a partir das minhas fotos, que pode ser alterada futuramente. Minhas fotos ficarão públicas e não existem anexos relacionados à elas, portanto me resta preencher os metadados. 
...
?> _TODO_ Continuar tutorial e subir imagens.