...
```php
public save_terms_metadatum_value( $item_metadata): mixed
```
...
```php
public add_compound_value(\Tainacan\Entities\Item_Metadata_Entity $item_metadata, mixed $meta_id): null|\Tainacan\Repositories\ind
```
...
```php
private extract_compound_value(array $ids, \Tainacan\Entities\Item $item,  $compound_meta_id): array
```
...
```php
public update( $object, null $new_values = null): mixed
```
